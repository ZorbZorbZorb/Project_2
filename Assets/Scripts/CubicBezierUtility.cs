...
namespace Assets.Scripts {
    ...
    public static class CubicBezierUtility {
        ...
        public static Vector2 EvaluateCurve (Vector2[] points, float t) {
        ...
        public static Vector2 EvaluateCurveDerivative (Vector2[] points, float t) {
            return EvaluateCurveDerivative (points[0], points[1], points[2], points[3], t);
        }

        /// Calculates the derivative of the curve at time 't'
        /// This is the vector tangent to the curve at that point
        public static Vector2 EvaluateCurveDerivative (Vector2 a1, Vector2 c1, Vector2 c2, Vector2 a2, float t) {
            t = Mathf.Clamp01 (t);
            return 3 * (1 - t) * (1 - t) * (c1 - a1) + 6 * (1 - t) * t * (c2 - c1) + 3 * t * t * (a2 - c2);
        }

        /// Returns the second derivative of the curve at time 't'
        public static Vector2 EvaluateCurveSecondDerivative (Vector2[] points, float t) {
            return EvaluateCurveSecondDerivative (points[0], points[1], points[2], points[3], t);
        }

        ///Returns the second derivative of the curve at time 't'
        public static Vector2 EvaluateCurveSecondDerivative (Vector2 a1, Vector2 c1, Vector2 c2, Vector2 a2, float t) {
            t = Mathf.Clamp01 (t);
            return 6 * (1 - t) * (c2 - 2 * c1 + a1) + 6 * t * (a2 - 2 * c2 + c1);
        }

        /// Calculates the normal vector (vector perpendicular to the curve) at specified time
        public static Vector2 Normal (Vector2[] points, float t) {
            return Normal (points[0], points[1], points[2], points[3], t);
        }

        /// Calculates the normal vector (vector perpendicular to the curve) at specified time
        public static Vector2 Normal (Vector2 a1, Vector2 c1, Vector2 c2, Vector2 a2, float t) {
            Vector2 tangent = EvaluateCurveDerivative (a1, c1, c2, a2, t);
            Vector2 nextTangent = EvaluateCurveSecondDerivative (a1, c1, c2, a2, t);
            Vector2 c = Vector3.Cross (nextTangent, tangent);
            return Vector3.Cross (c, tangent).normalized;
        }

        /// Splits curve into two curves at time t. Returns 2 arrays of 4 points.
        public static Vector2[][] SplitCurve (Vector2[] points, float t) {
            ...
            Vector2 pointOnCurve = Vector2.Lerp (b1, b2, t);

            return new Vector2[][] {
                new Vector2[] { points[0], a1, b1, pointOnCurve },
                    new Vector2[] { pointOnCurve, b2, a3, points[3] }
            };
        }

        // Crude, but fast estimation of curve length.
        public static float EstimateCurveLength (Vector2 p0, Vector2 p1, Vector2 p2, Vector2 p3) {
