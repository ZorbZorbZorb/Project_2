...
namespace Assets.Scripts {
    ...
    public class BezierPath {
        ...
        public BezierPath(Vector2 centre, bool isClosed = false, PathSpace space = PathSpace.xyz) {

            Vector2 dir = ( space == PathSpace.xz ) ? Vector3.forward : Vector3.up;
            float width = 2;
            float controlHeight = .5f;
            float controlWidth = 1f;
            points = new List<Vector2> {
                centre + Vector2.left * width,
                centre + Vector2.left * controlWidth + dir * controlHeight,
                centre + Vector2.right * controlWidth - dir * controlHeight,
                centre + Vector2.right * width
            };
        ...
        ///<param name="isClosed"> Should the end point connect back to the start point? </param>
        ///<param name="space"> Determines if the path is in 3d space, or clamped to the xy/xz plane </param>
        public BezierPath(IEnumerable<Vector2> points, bool isClosed = false, PathSpace space = PathSpace.xy) {
            ...
            if ( pointsArray.Length < 2 ) {
                ...
                this.points = new List<Vector2> { pointsArray[0], Vector2.zero, Vector2.zero, pointsArray[1] };
        ...
        /// Get world space position of point
        public void SetPoint(int i, Vector2 localPosition, bool suppressPathModifiedEvent = false) {
        ...
        public void AddSegmentToEnd(Vector2 anchorPos) {
            if ( isClosed ) {
                return;
            }
            ...
            Vector2 secondControlForOldLastAnchorOffset = ( points[lastAnchorIndex] - points[lastAnchorIndex - 1] );
            ...
            Vector2 secondControlForOldLastAnchor = points[lastAnchorIndex] + secondControlForOldLastAnchorOffset;
            Vector2 controlForNewAnchor = ( anchorPos + secondControlForOldLastAnchor ) * .5f;
        ...
        public void AddSegmentToStart(Vector2 anchorPos) {
            if ( isClosed ) {
                return;
            }

            // Set position for new control to be mirror of its counterpart
            Vector2 secondControlForOldFirstAnchorOffset = ( points[0] - points[1] );
            if ( controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic ) {
                // Set position for new control to be aligned with its counterpart, but with a length of half the distance from prev to new anchor
            ...
            Vector2 secondControlForOldFirstAnchor = points[0] + secondControlForOldFirstAnchorOffset;
            Vector2 controlForNewAnchor = ( anchorPos + secondControlForOldFirstAnchor ) * .5f;
        ...
        public void SplitSegment(Vector2 anchorPos, int segmentIndex, float splitTime) {
            ...
            if ( controlMode == ControlMode.Automatic ) {
                ...
                Vector2[][] splitSegment = CubicBezierUtility.SplitCurve(GetPointsInSegment(segmentIndex), splitTime);
                points.InsertRange(segmentIndex * 3 + 2, new Vector2[] { splitSegment[0][2], splitSegment[1][0], splitSegment[1][1] });
        ...
        public Vector2[] GetPointsInSegment(int segmentIndex) {
            ...
            return new Vector2[] { this[segmentIndex * 3], this[segmentIndex * 3 + 1], this[segmentIndex * 3 + 2], this[LoopIndex(segmentIndex * 3 + 3)] };
        }

        /// Move an existing point to a new position
        public void MovePoint(int i, Vector2 pointPos, bool suppressPathModifiedEvent = false) {
            Vector2 deltaMove = pointPos - points[i];
                ...
                if ( controlMode == ControlMode.Automatic ) {
                    ...
                    // If not in free control mode, then move attached control point to be aligned/mirrored (depending on mode)
                        ...
                        if ( attachedControlIndex >= 0 && attachedControlIndex < points.Count || isClosed ) {
                            ...
                            Vector2 dir = ( points[LoopIndex(anchorIndex)] - pointPos ).normalized;
        ...
        void AutoSetStartAndEndControls() {
            if ( isClosed ) {
                ...
                if ( NumAnchorPoints == 2 ) {
                    Vector2 dirAnchorAToB = ( points[3] - points[0] ).normalized;
                    float dstBetweenAnchors = ( points[0] - points[3] ).magnitude;
                    Vector2 perp = Vector3.Cross(dirAnchorAToB, ( space == PathSpace.xy ) ? Vector3.forward : Vector3.up);
        ...
        void UpdateClosedState() {
            if ( isClosed ) {
                ...
                Vector2 lastAnchorSecondControl = points[points.Count - 1] * 2 - points[points.Count - 2];
                Vector2 firstAnchorSecondControl = points[0] * 2 - points[1];
                if ( controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic ) {
